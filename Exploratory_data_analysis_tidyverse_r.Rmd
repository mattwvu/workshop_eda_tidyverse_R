...
coalProd <- mutate(coalProd, Mine_Type_num = recode_factor(Mine_Type, "Refuse" = 1, "Surface" = 2, "Underground" = 3, .ordered = F))
...
bigfoot_filter <- bigfoot %>% 
  mutate(title = ifelse(is.na(title), 999, title)) %>% 
  filter(title != "999")
...
coalProd_total_by_state <- coalProd %>% group_by(Mine_State) %>% summarise(total_production = sum(Production_short_tons))

coalProd_total_by_state
